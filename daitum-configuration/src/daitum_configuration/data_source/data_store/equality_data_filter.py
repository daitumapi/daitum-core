...
""":class:`EqualityDataFilter` — a :class:`DataFilter` matching exact-equality rows."""
...
@json_type_info(DataFilterType.EQUALITY.value)
@typechecked
class EqualityDataFilter(DataFilter):
    ...
    Args:
        path: Field path within each source row (e.g. ``["address", "country"]``).
        source_key: Model named value supplying the comparison value.
        value: Optional literal value emitted alongside the resolved key.
    """
